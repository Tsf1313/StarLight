...
CREATE TABLE schedules (
    ...
    status VARCHAR(50) DEFAULT 'upcoming' -- 'completed', 'active', 'upcoming'
);
...
CREATE TABLE feedback_submissions (
    ...
    event_id VARCHAR(50) REFERENCES events(id) ON DELETE CASCADE,
    name VARCHAR(255),
    role VARCHAR(50), -- 'Attendee', 'Volunteer', 'Speaker'
...
CREATE TABLE activity_logs (
    ...
    icon_type VARCHAR(50), -- e.g., 'user-check', 'trophy'
    color_theme VARCHAR(50), -- e.g., 'success', 'warning'
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 11. Table for Host Authentication
CREATE TABLE IF NOT EXISTS users (
    id VARCHAR(50) PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    email VARCHAR(255) UNIQUE NOT NULL,
    password VARCHAR(255) NOT NULL, -- Reminder: In production, use hashed passwords
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);